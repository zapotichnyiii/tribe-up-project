...
app = Flask(__name__)
# Секретний ключ для сесій (можна змінити на будь-який)
app.config['SECRET_KEY'] = 'local_dev_secret_key_123'
CORS(app)
...
app.config['MAIL_PASSWORD'] = 'pqehthpnvlduhyef'
...
def get_db():
    conn = psycopg2.connect(
        dbname='tribeup_db',  # Ім'я бази, яку ти створив
        user='postgres',      # Стандартний юзер postgres
        password='qazwsxedcasd123', # <--- ЗМІНИ НА СВІЙ ЛОКАЛЬНИЙ ПАРОЛЬ
        host='localhost',
        port=5432
    )
    return conn
...
# Запуск фонового потоку для нагадувань
...
def register():
    ...
    try:
        ...
        try:
            ...
            print(f"Mail Error (ignored for local dev if config missing): {e}") 
...
def get_events():
    ...
    cur = conn.cursor(cursor_factory=RealDictCursor)
    try:
        cur.execute("UPDATE events SET status = 'finished' WHERE event_date::timestamp < NOW() AND status = 'active'")
        conn.commit()
...
def get_event_participants(event_id):
    ...
    try:
        cur.execute("""
            ...
            WHERE ep.event_id = %s
        """, (event_id,))
        participants = [{'id': r['id'], 'name': r['name'], 'username': r['username'], 'avatarBase64': r['avatar_base64']} for r in cur.fetchall()]
...
def search_users():
    ...
    try:
        ...
        cur.execute("""
            ...
            FROM users WHERE is_verified = TRUE AND (username ILIKE %s OR name ILIKE %s) LIMIT 20
        """, (search_pattern, search_pattern))
        return jsonify([map_user(row) for row in cur.fetchall()])
    finally:
        conn.close()
...
def on_disconnect():
    ...
    if disconnected_user_id:
        ...
        try:
            conn = get_db()
            conn.cursor().execute("UPDATE users SET last_seen = NOW() WHERE id = %s", (disconnected_user_id,))
            conn.commit()
            conn.close()
        except: pass
